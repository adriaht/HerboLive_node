...
module.exports = function(pool) {
  ...
  return {
    ...
    // actualizar por id con objeto de claves/valores (solo las keys provistas)
    async updateById(id, updates = {}) {
      if (!id) throw new Error('updateById requires id');
      const keys = Object.keys(updates);
      if (!keys.length) return false;
      const sets = keys.map(k => `\`${k}\` = ?`).join(', ');
      const params = keys.map(k => updates[k]);
      params.push(id);
      const sql = `UPDATE plants SET ${sets} WHERE id = ?`;
      const r = await rawQuery(sql, params);
      // rawQuery puede devolver rows o resultado; comprobación básica
      try {
        if (r && typeof r.affectedRows !== 'undefined') return r.affectedRows >= 0;
      } catch(e){}
      return true;
    },

    // marcar como enriched y opcionalmente actualizar ciertos campos
    async markAsEnriched(id, updates = {}) {
      const data = Object.assign({}, updates, { enriched: 1 });
