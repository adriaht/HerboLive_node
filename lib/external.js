// /opt/herbolive/HerboLive_node/lib/external.js
'use strict';

/**
 * Implementación de readCsvLocal robusta y retrocompatible.
 * - intenta usar csv-parse/sync si está disponible
 * - si no está disponible, usa el parser robusto línea-a-línea
 *
 * Conserva el resto de helpers (mapCsvRecordToPlant, parseMaybeList, etc.)
 */
...
let parseSync = null;
try {
  // Si la dependencia existe y exporta .parse, la usamos
  // Algunas versiones exportan directamente la función, otras bajo .parse
  const csvParseMod = require('csv-parse/sync');
  parseSync = csvParseMod && (csvParseMod.parse || csvParseMod);
} catch (e) {
  // no disponible: seguiremos con el parser robusto (own implementation)
  // no hacer nada aquí, parseSync queda null
}

/**
 * Export del módulo: recibe config y devuelve un objeto API.
 * NOTA: este archivo implementa readCsvLocal y helpers asociados.
 */
module.exports = function(config = {}) {
  const API = {};

  // Placeholder: si tuvieras funciones fetchFromPerenual/fetchFromTrefle,
  // deben quedar definidas en este objeto. Las dejo como comentarios para
  // recordarte dónde integrarlas.
  //
  // API.fetchFromPerenual = async function(...) { ... }
  // API.fetchFromTrefle = async function(...) { ... }

  API.readCsvLocal = async function(maxRows) {
    // soportar diferentes nombres de config: csvMaxRead o CSV_MAX_READ
    const maxDefault = (config && (config.csvMaxRead || config.CSV_MAX_READ)) || 52;
    const limit = typeof maxRows === 'number' ? maxRows : maxDefault;

    try {
      // comprobar ruta absoluta o relativa
      ...
      // 1) Si parseSync está disponible, intentarlo primero
      if (parseSync) {
        try {
          ...
          // caer al fallback robusto
        }
      } else {
        if (config.DEBUG_SHOW_RAW) console.warn('csv-parse/sync no disponible: usando fallback robusto para parsear CSV');
      }

      // 2) Fallback robusto (línea-a-línea)
      const robust = robustParseCsv(raw, limit);
      return robust.map(rec => mapCsvRecordToPlant(rec));

    } catch (err) {
      console.error('readCsvLocal error', err && err.message ? err.message : err);
      return [];
    }
  };
  ...
  function mapCsvRecordToPlant(r) {
    ...
    const mapped = {
      ...
      pH_split: get(r, ['pH_split','p_h_split','pH_split']),
